...
my_parser.add_argument('--checkpoint_path',
                       type=str,
                       default="/home/smuthi2s/perl5/RnD-Weakly-Supervised-Object-Localization/Experiments/New_Experiment/weights/Baseline_local_at_work_classification_new_network_16_16_vgg16_150.pt",
...
my_parser.add_argument('--dataset_path',
                       type=str,
                       default = '/scratch/smuthi2s/split_at_work/images/',
                       help='Give the path to dataset')
my_parser.add_argument('--masks_path',
                       type=str,
                       default = '/scratch/smuthi2s/at_work/',
...
filename = [glob.glob(root+i+"/*.png") for i in object_categories]
filename_list = [item for sublist in filename for item in sublist]
filenames = [fn.split(os.sep)[-1].split('.')[0]+'.'+fn.split(os.sep)[-1].split('.')[1] for fn in filename_list]
# print(filenames)
count = 0
loc_accuracy_30=0
loc_accuracy_50=0
...
grad_cam = GradCam(model,
                       ...
                       use_cuda=False)
for img_path,fn in zip(filename_list,filenames):
    img_pil = Image.open(img_path)
    seg_img_path = args.masks_path+"labels/{}.png".format(fn)
    seg_img = np.array(Image.open(seg_img_path))
    an_path = args.masks_path+'new_labels/{}.xml'.format(fn)
...
#             print(a)
            ...
            bbox_real = [int(float(i)) for i in temp]
#             print(bbox_real)
            break
#     print(object_categories[idx[0]])
    if cor_pred:
